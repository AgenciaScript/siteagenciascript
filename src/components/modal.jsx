...
const Modal = ({ isOpen, onClose, name, description, imageUrl, linkedInUrl, email }) => {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4 transition-opacity duration-300 ease-in-out">
            ...
            <div className="bg-white rounded-lg shadow-lg max-w-2xl w-full p-6 md:p-8 z-60 flex flex-col md:flex-row relative transform transition-transform duration-300 ease-in-out scale-95">
                <button className="absolute top-2 right-2 text-gray-500 hover:text-gray-700" onClick={onClose}>
                    <X size={24} />
                </button>
                {loadedImage ? (
                    <img src={loadedImage} alt={`${name}`} className="w-full md:w-40 h-56 object-cover rounded-xl mb-4 md:mb-0 md:mr-6" />
                ) : (
                    <div className="w-full md:w-40 h-56 bg-gray-200 animate-pulse rounded-xl mb-4 md:mb-0 md:mr-6"></div>
                )}
                <div>
